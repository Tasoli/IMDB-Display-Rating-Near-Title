...
var addRatingToMovieRow = function(row, callback) {
  ...
  $(row).find('a[href*=title]:first').each(function(i, anchor) {
    ...
    // The API key "apikey=2ec174e2" is tied to my email address and is limited to 1,000 calls per day.
    $.get('http://www.omdbapi.com/?i=' + id + '&apikey=2ec174e2', function(response, xhr) {
      var data = response;
